...
if (typeof ARIA2=="undefined"||!ARIA2) var ARIA2=(function(){
  ...
  return {
    ...
    change_option: function(gid, options) {
      ARIA2.request("getFiles", [gid, options],
        function(result) {
			//alert(result.);
          if (!result.result) {
            main_alert("alert-error", "<strong>Error: </strong>rpc result error.", 5000);
		  ...
		  if (result.result.length != 1)
		  	main_alert("alert-error", "<strong>Error: </strong>uri length error.", 5000);
			else
			{
				// HXX
				var path = result.result[0].path;
				var global_save_dir = "/mnt/usb/img/public/download";
				var truepath = path.replace(global_save_dir, "/download");
				window.location.href = truepath;
			}
        }
      );
    },
